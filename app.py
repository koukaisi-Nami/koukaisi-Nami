...
会社ルールが保存済みskillsにあれば優先。ただし仲介手数料だけは例外で、ユーザーが今回明示指定した場合だけ金額を入れ、指定がなければ必ず「－」とする。図面や会社ルールから仲介手数料を自動補完しない。
...
def three_document_command(text,uid,cid,qid=None):
    ...
    if kind=="estimate" and not wants_image and not wants_pdf:
        prompt=("募集図面の読取結果から、お客様へそのままLINE転送できる初期費用の概算を作成してください。短く見やすく丁寧にし、Markdown表、計算過程、内部事情、前回回答への言及は禁止です。\n"
                ...
                "仲介手数料はユーザーが明示指定した場合だけ金額を入れてください。『なし』『無料』『0円』は0円、『5万円』などの円指定はその金額、『0.5ヶ月』『1ヶ月』などの月数指定はその指定に従って計算してください。ユーザーから今回の見積もりについて仲介手数料の指定がない場合は、図面記載・一般的な相場・保存済み会社ルール・過去の見積もりに金額があっても必ず「－」としてください。仲介手数料を自動で家賃1ヶ月＋税等にしないでください。\n"
