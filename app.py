...
def three_document_command(text,uid,cid,qid=None):
    ...
    if kind=="estimate" and not wants_image and not wants_pdf:
        prompt=("募集図面の読取結果から、お客様へそのままLINE転送できる初期費用の概算を作成してください。短く見やすく丁寧にし、Markdown表、計算過程、内部事情、前回回答への言及は禁止です。\n"
                ...
                "仲介手数料は今回のユーザー発言で『0.5ヶ月』『なし』『5万円』など明示指定した場合だけ計上してください。今回指定がなければ、図面・保存済み会社ルール・過去見積に金額があっても必ず「－」。自動で1ヶ月＋税等にしないでください。\n"
                "火災保険等も不明なら「－」。合計は金額が確定している項目だけをPythonで足した場合と整合するよう計算し、不明項目を含めないでください。末尾は必ず『※入居日・未確定項目により金額が変動します。』だけにしてください。")
        if qid:
