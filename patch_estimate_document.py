...
# Explicit output only: ordinary estimate remains text.
...
if 'def estimate_file_download(' not in s:
    insert='''ESTIMATE_CACHE={}\n\n@app.get("/estimate-file/<key>.<ext>")\ndef estimate_file_download(key,ext):\n    row=ESTIMATE_CACHE.get(key)\n    if not row:return "not found",404\n    created,text=row\n    if time.time()-created>600:\n        ESTIMATE_CACHE.pop(key,None); return "expired",404\n    if ext=="pdf": return send_file(make_estimate_document(text),mimetype="application/pdf",as_attachment=True,download_name="見積もり概算書.pdf")\n    if ext=="png": return send_file(make_estimate_image(text),mimetype="image/png")\n    return "not found",404\n\ndef reply_artifact(tok,url,kind):\n    if kind=="image": msg={"type":"image","originalContentUrl":url,"previewImageUrl":url}\n    else: msg={"type":"file","originalContentUrl":url,"fileName":"見積もり概算書.pdf"}\n    try:\n        r=requests.post("https://api.line.me/v2/bot/message/reply",headers={"Authorization":f"Bearer {TOKEN}","Content-Type":"application/json"},json={"replyToken":tok,"messages":[msg]},timeout=30)\n        if not r.ok: print("LINE artifact reply",r.status_code,r.text[:1000],flush=True)\n        return r.ok\n    except Exception as x: print("reply_artifact",repr(x),flush=True); return False\n\n'''+anchor
    if anchor not in s: raise SystemExit('health anchor missing')
    s=s.replace(anchor,insert,1)

old_handler='''            if isinstance(quick_doc,tuple) and quick_doc[0]=='__ESTIMATE_IMAGE__':\n                ans='見積もり画像用の内容を作成したよ🧭\\n'+quick_doc[1]+'\\n※画像は固定テンプレート描画で金額を変えずに生成する仕様です。\n            elif isinstance(quick_doc,tuple) and quick_doc[0]=='__PDF__':'''
new_handler='''            if isinstance(quick_doc,tuple) and quick_doc[0] in ('__ESTIMATE_IMAGE__','__ESTIMATE_PDF__'):\n                estimate_text=quick_doc[1]\n                key=hashlib.sha256((eid+str(time.time())).encode()).hexdigest()[:24]\n                ESTIMATE_CACHE[key]=(time.time(),estimate_text)\n                is_image=quick_doc[0]=='__ESTIMATE_IMAGE__'; ext='png' if is_image else 'pdf'\n                base=os.getenv("PUBLIC_BASE_URL","https://koukaisi-nami.onrender.com").rstrip('/')\n                file_url=f"{base}/estimate-file/{key}.{ext}"\n                save_msg("assistant:"+eid,cid,"bot","航海士ナミ","assistant",estimate_text)\n                reply_artifact(e.get("replyToken"),file_url,'image' if is_image else 'pdf')\n                continue\n            elif isinstance(quick_doc,tuple) and quick_doc[0]=='__PDF__':'''
if old_handler in s:
    s=s.replace(old_handler,new_handler,1)
elif 'reply_artifact(e.get("replyToken")' not in s:
