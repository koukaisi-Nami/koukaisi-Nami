...
def media_ai(blob,mime,uid,cid,question=""):
    """Analyze images/PDF directly. Only called when media actually needs AI."""
    q=(question or "").strip() or "この資料を詳細に読み取ってください。"
    if mime and ("pdf" in mime.lower()):
        parts=[
          {"type":"input_text","text":ctx(uid,cid,q)+"\\n【今回】\\n"+q[:3000]},
          {"type":"input_file","filename":"document.pdf","file_data":"data:application/pdf;base64,"+base64.b64encode(blob).decode()}
        ]
        ...
        try:
            ...
            if not r.ok:
                print("OPENAI_MEDIA",r.status_code,r.text[:2000],flush=True); return f"PDF解析エラー({r.status_code})"
            ...
            print("media_ai_pdf",repr(x),flush=True); return "PDF解析で接続エラー"
...
賃貸募集図面なら物件名、号室、所在地、交通、間取り、面積、賃料、管理費、敷金、礼金、保証金、契約期間、
保証会社、初回/月額保証料、火災保険、鍵交換、24時間サポート、クリーニング、その他初期/月額費用、
入居日、設備、特約、AD等を抽出。見積に使える金額を項目別に構造化し、必須/任意・税込/税別も分かる範囲で区別。
数字は推測しない。不鮮明・未記載は必ず「要確認」。"""
...
# Cost optimization: web search only when freshness/current public info is likely needed.
