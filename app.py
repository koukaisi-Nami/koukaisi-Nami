...
def three_document_command(text,uid,cid,qid=None):
    ...
    if kind=="estimate" and not wants_image and not wants_pdf:
        prompt=("募集図面の読取結果から、お客様へそのままLINE転送できる初期費用の概算を作成してください。短く見やすく丁寧にし、Markdown表、計算過程、内部事情、前回回答への言及は禁止です。\n"
                ...
                "【保険判定・最優先】「火災保険」という名称だけで探さない。損保、損害保険、家財保険、家財、住宅保険、借家人賠償責任保険、少額短期保険、保険料、保険加入など、住宅・家財・借家人賠償の保険を火災保険として扱う。図面に「損保 要2万円2年」のような記載があれば、火災保険：20,000円として計上する。金額表記は「2万円」「20,000円」「20000円」等を同じ金額として読む。保険加入の記載はあるが金額がない場合、または保険相当の記載自体がない場合は、必ず「火災保険（仮）：20,000円」と表示し20,000円を合計に含める。火災保険を「－」にしてはいけない。【表記ゆれ】敷金/保証金/契約保証金/預り金、初回保証料/保証委託料/初回委託保証料/保証会社利用料、24時間サポート/安心サポート/緊急サポート/入居者サポート、鍵交換/鍵交換代/鍵設定費/シリンダー交換等は名称完全一致ではなく意味と文脈で分類する。既存カテゴリに当てはまらない契約時の金銭項目も捨てず、図面の名称のまま事務手数料の後・合計の前に追加する。保証金は敷金相当か独立費用か文脈で判定し二重計上しない。【保証料fallback】保証会社の初回保証料の金額・率が資料にない場合は、賃料＋管理費・共益費の50%を「初回保証料（仮）」として表示し合計に含める。【合計】「－」以外に表示した全ての金額（仮計上・次月前家賃・追加費用を含む）を必ず合計へ加算し、回答前に表示金額の総和と合計が一致するか再検算する。末尾は『※入居日・未確定項目により金額が変動します。』とする。")
        if qid:
